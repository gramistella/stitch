...
fn transform_functions_to_signatures(src: &str) -> String {
    ...
    fn scan_str(b: &[u8], n: usize, mut j: usize, q: u8) -> usize {
        let mut esc = false;
        j += 1;
        while j < n {
            let c = b[j];
    ...
    fn scan_raw(b: &[u8], n: usize, mut j: usize) -> usize {
        let mut k = j + 1;
        let mut h = 0;
        while k < n && b[k] == b'#' {
            h += 1;
                    ...
                    if prev_ok && next_ok {
                        ...
                        while j < n {
                            match st2 {
                                ...
                                S::Dq { .. } => {
                                    j = scan_str(b, n, j.saturating_sub(1), b'"');
                                    st2 = S::Code;
                                }
                                S::Sq { .. } => {
                                    j = scan_str(b, n, j.saturating_sub(1), b'\'');
                                ...
                                S::Raw { .. } => {
                                    j = scan_raw(b, n, j.saturating_sub(1));
                                    st2 = S::Code;
                                }
                            }
                        }
                        ...
                        let mut sig_end = j;
                        while sig_end > i && b[sig_end - 1].is_ascii_whitespace() {
                            sig_end -= 1;
                        }
                        ...
                        while k < n && depth > 0 {
                            match st3 {
                                ...
                                S::Dq { .. } => {
                                    k = scan_str(b, n, k.saturating_sub(1), b'"');
                                    st3 = S::Code;
                                }
                                S::Sq { .. } => {
                                    k = scan_str(b, n, k.saturating_sub(1), b'\'');
                                ...
                                S::Raw { .. } => {
                                    k = scan_raw(b, n, k.saturating_sub(1));
                                    st3 = S::Code;
                                }
                            }
                        }
...
fn remove_comments_textual(input: &str, remove_inline: bool, remove_doc: bool) -> String {
    ...
    enum S {
        Code,
        BlockComment { remove: bool, depth: usize },
        Dq { esc: bool },
        Sq { esc: bool },
        Raw { hashes: usize },
    }
